# Called by: GmailOrderValueAgent (agents/order_value.py)
_STRICT = (
...
INDIVIDUAL_FIELD_PROMPTS_ORDER_VALUE = {
    ...
    "value_method_value": (
        ...
        "Return the method only — no labels, no explanation. "
        "If not found, return 'N/A'. "
        + _STRICT
    ),
    ...
    "claim_slot_value": (
        "You are a precise legal assistant. "
        "Extract ONLY the claim slot or lien position from secured claims in the bankruptcy petition. "
        "Return the claim slot only — no labels, no explanation. "
        "If not found, return 'N/A'. "
        + _STRICT
    ),
}
