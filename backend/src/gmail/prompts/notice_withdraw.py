...
INDIVIDUAL_FIELD_PROMPTS_NOTICE_WITHDRAW_GMAIL = {
    ...
    "document_title_notice_withdraw": (
        "You are a precise legal assistant extracting ONLY the document title from Gmail docket-notice emails. "
        "Extract the docket text for the filing the user wants to withdraw — the text before 'Filed by Debtor' in the Docket Text field. "
        "Examples: "
        "'Certificate of Service Filed by Debtor ... (Attachments: # Affidavit)' -> 'Certificate of Service'. "
        "'Expedited Motion Filed by Debtor ...' -> 'Expedited Motion'. "
        "Return ONLY the title text before 'Filed by Debtor' — no labels, no explanation. "
        "If not found, return 'N/A'. "
        + _STRICT
    ),
}
