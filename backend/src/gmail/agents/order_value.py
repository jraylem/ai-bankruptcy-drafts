...
from ..prompts import (
    INDIVIDUAL_FIELD_PROMPTS_ORDER_VALUE,
)


# Called by: service.generate_order_value_payload_for_session_gmail (L3A -> routes/order_stream.py)
class GmailOrderValueAgent:
    """
    Claude Sonnet-backed Order Value Agent.
    ...
    def _get_optimized_query(self, field_name: str, user_hint: str = None) -> str:
        field_queries = {
            ...
            "claim_slot_value": "claim slot position secured",
        }
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                agent_executor = create_react_agent(
                    tools=[field_tool],
                    model=self.llm,
                    prompt=INDIVIDUAL_FIELD_PROMPTS_ORDER_VALUE[field_name],
                )
    ...
    def extract_payload(self, user_hint: Optional[str] = None) -> Dict[str, Any]:
        """
        Extract order value payload using Gmail + petition vectorstores via Claude Haiku.
        """
        try:
            print("Starting Claude-backed sequential field extraction for order value...")
            ...
            final_payload = {
                ...
                "Select1": self.static_fields["Select1"],
                ...
                "Select2": self.static_fields["Select2"],
                "Value": pdf_results.get("value_amount_value", "N/A"),
                "ValueMethod": pdf_results.get("value_method_value", "N/A"),
                "Put1": self.static_fields["Put1"],
                "Percent1": self.static_fields["Percent1"],
                "Price1": self.static_fields["Price1"],
                "Put2": self.static_fields["Put2"],
                "Percent2": self.static_fields["Percent2"],
                "Price2": self.static_fields["Price2"],
                "ClaimSlot": pdf_results.get("claim_slot_value", "N/A"),
            }
            ...
            print(f"Final Claude-backed order value payload: {final_payload}")
            print(f"Success rate: {success_rate:.1f}% ({successful_fields}/{total_fields} fields)")

            return {
                "payload": final_payload,
                "status": "completed",
                "agent_type": "claude_order_value_agent_sequential",
            ...
            return {
                "payload": f"Claude Order value payload extraction failed: {str(e)}",
                "status": "failed",
                "error": str(e),
                "agent_type": "claude_order_value_agent_sequential",
            }
