...
from ..tools import order_extension_gmail_tool
from ..prompts import INDIVIDUAL_FIELD_PROMPTS_ORDER_EXTENSION


# Called by: service.generate_order_extension_payload_for_session_gmail (routes/order_stream.py)
class GmailOrderExtensionAgent:
    """
    Claude Sonnet-backed Order Extension Agent.
    ...
    def __init__(self, session_id: Optional[str] = None, memory_saver: MemorySaver = None):
        ...
        self.pdf_fields = ["debtor_name_extension"]
        self.gmail_fields = ["case_number_extension", "chapter_extension"]

    def _get_optimized_query(self, field_name: str, user_hint: str = None) -> str:
        field_queries = {
            "debtor_name_extension": "Your full name Debtor 1",
            "case_number_extension": "Case number with judge initial from emails",
            "case_number_extension_pdf": "Case number if known",
            "chapter_extension": "chapter case details",
            "chapter_extension_pdf": "chapter case details",
        }
    ...
    def _extract_single_field(self, field_name: str, query: str) -> str:
        ...
        for attempt in range(max_retries + 1):
            try:
                tools = order_extension_gmail_tool(session_id=self.session_id)
                field_tool = None
                ...
                agent_executor = create_react_agent(
                    tools=[field_tool],
                    model=self.llm,
                    prompt=INDIVIDUAL_FIELD_PROMPTS_ORDER_EXTENSION[field_name],
                )
    ...
    def extract_payload(self, user_hint: Optional[str] = None) -> Dict[str, Any]:
        """
        Extract order extension payload using Gmail + petition vectorstores.
        """
        try:
            print("Starting sequential field extraction for order extension...")
            ...
            case_number = gmail_results.get("case_number_extension", "N/A")
            if not case_number or case_number == "N/A":
                print("  case_number_extension N/A from Gmail, falling back to PDF...")
                case_number = self._extract_single_field(
                    "case_number_extension_pdf",
                    self._get_optimized_query("case_number_extension_pdf"),
                )
                print(f"    case_number_extension (PDF fallback): {case_number}")

            chapter = gmail_results.get("chapter_extension", "N/A")
            if not chapter or chapter == "N/A":
                print("  chapter_extension N/A from Gmail, falling back to PDF...")
                chapter = self._extract_single_field(
                    "chapter_extension_pdf",
                    self._get_optimized_query("chapter_extension_pdf"),
                )
                print(f"    chapter_extension (PDF fallback): {chapter}")

            final_payload = {
                "CaseNumber":    case_number,
                "ChapterNumber": chapter,
                "DebtorName":    pdf_results.get("debtor_name_extension", "N/A"),
            }
            ...
            print(f"Final order extension base payload: {final_payload}")
            print(f"Success rate: {success_rate:.1f}% ({successful_fields}/{total_fields} fields)")

            return {
                "payload": final_payload,
                "status": "completed",
                "agent_type": "gmail_order_extension_agent_sequential",
            ...
            return {
                "payload": f"Order extension payload extraction failed: {str(e)}",
                "status": "failed",
                "error": str(e),
                "agent_type": "gmail_order_extension_agent_sequential",
            }
