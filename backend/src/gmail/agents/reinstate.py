...
# Called by: service/reinstate.py → generate_payload_reinstate_for_session_gmail
#   -> tasks/orchestrator.py, tasks/extractors.py
class GmailMotionReinstateAgent:
    """
    Claude-backed Motion Reinstate Agent.

    Uses:
    - Petition vectorstore (bankruptcy_knowledge_<session_id>) for:
      debtor_name_reinstate
    - Gmail vectorstore (gmail_<session_id>) for:
      case_number_reinstate_gmail (with judge initial), chapter_number_reinstate
    - PDF fallback for case number if Gmail returns N/A:
    ...
    def _extract_single_field(self, field_name: str, query: str) -> str:
        ...
        for attempt in range(max_retries + 1):
            try:
                tools = motion_reinstate_gmail_tool(session_id=self.session_id)
                field_tool = None
                ...
                agent_executor = create_react_agent(
                    tools=[field_tool],
                    model=self.llm,
                    prompt=INDIVIDUAL_FIELD_PROMPTS_REINSTATE_GMAIL[field_name],
                )
    ...
    def extract_payload(self, user_hint: Optional[str] = None) -> Dict[str, Any]:
        """
        Extract motion reinstate payload using Gmail + petition vectorstores.
        """
        try:
            print("Starting Claude-backed sequential field extraction for motion reinstate...")
            ...
            case_number = gmail_results.get("case_number_reinstate_gmail", "N/A")
            # PDF fallback: if case_number is N/A, extract from petition
            if not case_number or case_number == "N/A":
                print("  case_number_reinstate_gmail N/A from Gmail, falling back to PDF...")
                case_number = self._extract_single_field(
                    "case_number_reinstate",
                    self._get_optimized_query("case_number_reinstate"),
                )
                print(f"    case_number_reinstate (PDF fallback): {case_number}")

            chapter = gmail_results.get("chapter_number_reinstate", "N/A")
            ...
            print(f"Final Claude-backed motion reinstate payload: {final_payload}")
            print(
            ...
            return {
                ...
                "status": "completed",
                "agent_type": "claude_motion_reinstate_agent_sequential",
            ...
            return {
                "payload": f"Claude Motion reinstate payload extraction failed: {str(e)}",
                "status": "failed",
                "error": str(e),
                "agent_type": "claude_motion_reinstate_agent_sequential",
            }
