...
# Called by: service.generate_payload_LOE_for_session_gmail (L3D)
#   -> routes/stream.py
class GmailMotionLOEAgent:
    def __init__(
        self,
        ...
        """
        Initialize the Gmail-backed Motion LOE Agent for LOE motion payload.
        Uses:
        - Petition vectorstore (bankruptcy_knowledge_<session_id>) for debtor_name and case_number
        - Gmail vectorstore (gmail_<session_id>) for trustee_name, chapter_number, and judge_initial
        """
        ...
        # Gmail-based fields (trustee name, chapter number, judge initial)
        self.gmail_fields = [
            "trustee_name_loe",
            "chapter_number_loe",
            "judge_initial_loe",
    ...
    def _get_optimized_query(self, field_name: str, user_hint: str = None) -> str:
        field_queries = {
            "trustee_name_loe": "trustee name from emails",
            "chapter_number_loe": "chapter case details from emails",
            "judge_initial_loe": "case number with judge initial format xx-xxxxx-XXX",
            "debtor_name_loe": "Your full name Debtor 1",
            "case_number_loe": "Case number if known",
        }
    ...
    def _extract_single_field(self, field_name: str, query: str) -> str:
        ...
        for attempt in range(max_retries + 1):
            try:
                tools = loe_gmail_tool(session_id=self.session_id)
                field_tool = None
                ...
                agent_executor = create_react_agent(
                    tools=[field_tool],
                    model=self.llm,
                    prompt=INDIVIDUAL_FIELD_PROMPTS_LOE_GMAIL[field_name],
                )
    ...
    def extract_payload(self, user_hint: Optional[str] = None) -> Dict[str, Any]:
        """
        Extract LOE motion payload using Gmail + petition vectorstores.
        """
        try:
            print("Starting Gmail-backed sequential field extraction for motion LOE...")
            ...
            # Combine judge initial with case number
            judge_initial = gmail_results.get("judge_initial_loe", "N/A")
            case_number = pdf_results.get("case_number_loe", "N/A")
            ...
            final_payload = {
                "Date": current_date,
                "TrusteeName": gmail_results.get("trustee_name_loe", "N/A"),
                "ChapterNumb": gmail_results.get("chapter_number_loe", "N/A"),
                "DebtorName": pdf_results.get("debtor_name_loe", "N/A"),
                "CaseNumb": combined_case_number,
                "Explanation": self.static_fields["explanation"],
                "AttorneyName": self.static_fields["attorney_name"],
            }
            ...
            print(f"Final Gmail-backed LOE payload: {final_payload}")
            print(f"Success rate: {success_rate:.1f}% ({successful_fields}/{total_fields} fields)")

            return {
                "payload": final_payload,
                "status": "completed",
                "agent_type": "gmail_motion_LOE_agent_sequential",
                "field_results": {
                    **extracted_fields,
            ...
            return {
                "payload": f"Gmail Motion LOE payload extraction failed: {str(e)}",
                "status": "failed",
                "error": str(e),
                "agent_type": "gmail_motion_LOE_agent_sequential",
            }
