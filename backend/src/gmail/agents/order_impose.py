...
from ..tools import (
    order_impose_regular_gmail_tool,
)
from ..prompts import (
...
# Called by: service.generate_payload_order_impose_regular_for_session_gmail (L2 — unused caller)
#   -> only reached via L4 legacy functions; effectively unused
class GmailOrderImposeRegularAgent:
    """
    Gmail-backed Order Impose Regular Agent.
    ...
    - Gmail vectorstore (gmail_<session_id>) for:
      case_number_order_impose_regular (with judge initial suffix), chapter_order_impose_regular
    """
    ...
    def __init__(self, session_id: Optional[str] = None, memory_saver: MemorySaver = None):
        ...
        self.pdf_fields = ["debtor_name_order_impose_regular"]
        self.gmail_fields = ["case_number_order_impose_regular", "chapter_order_impose_regular"]
        self.static_fields = {
            "CalendarDate": "N/A",
            "Docket": "N/A",
            "granted": "N/A",
    ...
    def _get_optimized_query(self, field_name: str, user_hint: str = None) -> str:
        field_queries = {
            "debtor_name_order_impose_regular": "Your full name Debtor 1",
            "case_number_order_impose_regular": "case number if known with judge initials",
            "chapter_order_impose_regular": "chapter case details",
        }
    ...
    def _extract_single_field(self, field_name: str, query: str) -> str:
        ...
        for attempt in range(max_retries + 1):
            try:
                tools = order_impose_regular_gmail_tool(session_id=self.session_id)
                field_tool = None
                ...
                agent_executor = create_react_agent(
                    tools=[field_tool],
                    model=self.llm,
                    prompt=INDIVIDUAL_FIELD_PROMPTS_ORDER_IMPOSE_REGULAR_GMAIL[field_name],
                )
    ...
    def extract_payload(self, user_hint: Optional[str] = None) -> Dict[str, Any]:
        """
        Extract order impose regular payload using Gmail + petition vectorstores.
        """
        try:
            print("Starting Gmail-backed sequential field extraction for order impose regular...")
            ...
            case_number = gmail_results.get("case_number_order_impose_regular", "N/A")

            final_payload = {
                "DebtorName": pdf_results.get("debtor_name_order_impose_regular", "N/A"),
                "CaseNumber": case_number,
                "Chapter": gmail_results.get("chapter_order_impose_regular", "N/A"),
                "CalendarDate": self.static_fields["CalendarDate"],
                "Docket": self.static_fields["Docket"],
                "granted": self.static_fields["granted"],
                "OptionalConditions": self.static_fields["OptionalConditions"],
            }

            extracted_fields = {**pdf_results, **gmail_results, **self.static_fields}
            ...
            print(f"Final Gmail-backed order impose regular payload: {final_payload}")
            print(
            ...
            return {
                ...
                "status": "completed",
                "agent_type": "gmail_order_impose_regular_agent_sequential",
            ...
            return {
                "payload": f"Gmail Order impose regular payload extraction failed: {str(e)}",
                "status": "failed",
                "error": str(e),
                "agent_type": "gmail_order_impose_regular_agent_sequential",
            }
