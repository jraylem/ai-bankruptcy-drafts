...
# Called by: service.generate_payload_withdraw_for_session_gmail (L2)
#   -> routes/order_stream.py, routes/service_order_stream.py
class GmailMotionWithdrawAgent:
    """
    Gmail-backed Motion Withdraw Agent.
    ...
    def __init__(self, session_id: Optional[str] = None, memory_saver: MemorySaver = None):
        ...
        self.pdf_fields = ["debtor_name_withdraw", "debtor_current_addy_withdraw"]
        # Gmail-based fields (case number with judge, chapter, judge initial)
        self.gmail_fields = [
    ...
    def _get_optimized_query(self, field_name: str, user_hint: str = None) -> str:
        field_queries = {
            "debtor_name_withdraw": "Your full name Debtor 1",
            "debtor_current_addy_withdraw": "Where you live",
            "case_number_withdraw": "Case number with judge initial from emails",
            "chapter_withdraw": "chapter case details",
            "judge_initial_withdraw": "Judge",
            "case_number_withdraw_pdf": "Case number if known",
            "chapter_withdraw_pdf": "chapter case details",
        }
    ...
    def _extract_single_field(self, field_name: str, query: str) -> str:
        ...
        for attempt in range(max_retries + 1):
            try:
                tools = motion_withdraw_gmail_tool(session_id=self.session_id)
                field_tool = None
                ...
                agent_executor = create_react_agent(
                    tools=[field_tool],
                    model=self.llm,
                    prompt=INDIVIDUAL_FIELD_PROMPTS_WITHDRAW_GMAIL[field_name],
                )
    ...
    def extract_payload(self, user_hint: Optional[str] = None) -> Dict[str, Any]:
        """
        Extract motion withdraw payload using Gmail + petition vectorstores.
        """
        try:
            print("Starting Gmail-backed sequential field extraction for motion withdraw...")
            ...
            case_number = gmail_results.get("case_number_withdraw", "N/A")
            judge_initial = gmail_results.get("judge_initial_withdraw", "N/A")

            if case_number == "N/A":
                case_number = pdf_results

            # PDF fallback: if case_number is still N/A (or incorrectly set), extract from petition
            if not case_number or case_number == "N/A" or not isinstance(case_number, str):
                print("  case_number_withdraw N/A from Gmail, falling back to PDF...")
                case_number = self._extract_single_field(
                    "case_number_withdraw_pdf",
                    self._get_optimized_query("case_number_withdraw_pdf"),
                )
                print(f"    case_number_withdraw (PDF fallback): {case_number}")

            chapter = gmail_results.get("chapter_withdraw", "N/A")
            if not chapter or chapter == "N/A":
                print("  chapter_withdraw N/A from Gmail, falling back to PDF...")
                chapter = self._extract_single_field(
                    "chapter_withdraw_pdf",
                    self._get_optimized_query("chapter_withdraw_pdf"),
                )
                print(f"    chapter_withdraw (PDF fallback): {chapter}")

            # Create final JSON payload
            final_payload = {
                "DebtorName": pdf_results.get("debtor_name_withdraw", "N/A"),
                "CaseNumber": case_number,
                "Chapter": chapter,
                "Judge": judge_initial,
                "DebtorCurrentAddy": pdf_results.get("debtor_current_addy_withdraw", "N/A"),
            }
            ...
            print(f"Final Gmail-backed withdraw payload: {final_payload}")
            print(f"Success rate: {success_rate:.1f}% ({successful_fields}/{total_fields} fields)")

            return {
                "payload": final_payload,
                "status": "completed",
                "agent_type": "gmail_motion_withdraw_agent_sequential",
            ...
            return {
                "payload": f"Gmail Motion withdraw payload extraction failed: {str(e)}",
                "status": "failed",
                "error": str(e),
                "agent_type": "gmail_motion_withdraw_agent_sequential",
            }
