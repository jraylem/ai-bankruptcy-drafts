...
Mirrors `src/core/components/pleading/events.py` 1:1 with a composer-
specific key prefix and a smaller event-type set (no awaiting_input,
no existing_found, no resume).

Writes events to `core:composer:events:{user_id}` as a ring buffer
(XADD with MAXLEN ~ 500). The SSE generator in `sse.py` tails the
stream with `XREAD BLOCK` and forwards every entry to the connected
EventSource.

Durable across reconnects (`Last-Event-ID` resume) and worker
restarts. Pub/sub is NOT used — it's fire-and-forget and would lose
events during brief FE disconnects.
"""
...
STREAM_KEY_PREFIX = "core:composer:events:"
STREAM_MAXLEN = 500
...
async def emit(user_id: str, task_id: str, event_type: EventType) -> None:
    ...
    if record is None:
        logger.warning(
            "composer events: task %s not found, skipping %s event",
            task_id, event_type,
        )
        return

    payload = V2ComposerTaskResponse.from_record(record).model_dump_json()
    ...
    try:
        ...
        logger.info(
            "[composer-v2 emit] user=%s task=%s event=%s entry=%s status=%s kind=%s",
            user_id, task_id, event_type, entry_id, record.status, record.kind,
        )
    finally:
        await r.aclose()
