"""SSE endpoint generator for the v2 dry-run-async stream.

Tails the per-user Redis Stream (`core:dry_run:events:{user_id}`)
with a 15s blocking XREAD. On connect, emits a synthetic `snapshot`
event with the user's currently-tracked tasks so a cold reload
always rehydrates the rail section.

Mirrors `studio_v2/composer/async_run/sse.py` 1:1 with dry-run-side
...
async def decode_sse_token(token: str) -> User:
    ...
    try:
        ...
        logger.warning("dry-run SSE JWT decode failed: %s", exc)
        raise credentials_exception
...
async def stream_events(
    ...
    """Server-Sent Events handler for /api/v3/studio/dry-run-async/events."""
    ...
    logger.info(
        "[dry-run-v2 sse] connect user=%s key=%s last_event_id=%s",
    ...
    async def event_generator():
        try:
            ...
            # reconnect (same rationale as composer-async SSE).
            all_for_user = await state.list_for_user(user_id)
            tasks_payload = [
                V2DryRunTaskResponse.from_record(r).model_dump(mode="json")
                for r in all_for_user
            ]
            logger.info(
                "[dry-run-v2 sse] snapshot user=%s start_id=%s task_count=%d",
            ...
            while True:
                if await request.is_disconnected():
                    logger.info("[dry-run-v2 sse] disconnect user=%s", user_id)
                    break
                try:
                    ...
                    logger.exception(
                        "dry-run SSE XREAD failed on %s for user %s: %s",
                        key, user_id, e,
                    )
                ...
                for _, entries in result:
                    for entry_id, fields in entries:
                        ...
                        logger.info(
                            "[dry-run-v2 sse] yield user=%s entry=%s event=%s",
                            user_id, entry_id, event_type,
                        )
                        yield f"id: {entry_id}\nevent: {event_type}\ndata: {data}\n\n"
        except asyncio.CancelledError:
            logger.debug("dry-run SSE generator cancelled for user %s", user_id)
            raise
        except Exception as e:  # noqa: BLE001
            logger.exception("dry-run SSE generator failed for user %s: %s", user_id, e)
            raise
        finally:
            try:
