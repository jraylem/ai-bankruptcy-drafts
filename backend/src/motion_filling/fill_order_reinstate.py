...
TEMPLATE_CANDIDATES = [
    BASE_DIR.parent / "templates" / "order_motion_reinstate.docx",
]

OUTPUT_BASENAME = "Order_Motion_Reinstate_FILLED"


def ensure_docx_template() -> Path:
    """Return the first existing DOCX template for the order on motion to reinstate."""
    for p in TEMPLATE_CANDIDATES:
        if p.exists():
            return p
    raise SystemExit("Template not found. Place 'order_motion_reinstate.docx' under templates/ .")
...
#   DebtorName, CaseNumber, ChapterNumber, DocketNumber, TrusteeCalendar, X1, X2, X3
# Template fields:
#   DebtorName, CaseNumber, ChapterNumber, DocketNumber, TrusteeCalendar, X1, X2, X3
...
def _normalize_checkbox(val) -> str:
    """Return '✓' if val is truthy (True, 'yes', 'y', 'u', '1', 'x'), else ''."""
    if isinstance(val, bool):
        return "✓" if val else ""
    if val is None:
        return ""
    s = str(val).strip().lower()
    if s in {"true", "yes", "y", "u", "1", "x", "✓"}:
        return "✓"
    return ""

def build_context(ai: dict) -> dict:
    """
    Build context dictionary for Order on Motion to Reinstate template.
    """
    ...
    # X1/X2/X3 come directly from the payload (set by caller or front-end).
    # X1 - Provision A
    # X2 - Provision B
    # X3 - Provision C
    x1_raw = ai.get("X1")
    x2_raw = ai.get("X2")
    x3_raw = ai.get("X3")
    x1 = _normalize_checkbox(x1_raw)
    x2 = _normalize_checkbox(x2_raw)
    x3 = _normalize_checkbox(x3_raw)
...
def render_docx(template_docx: Path, ctx: dict, name_slug: str) -> Path:
    ...
    print(f"DEBUG: Rendering order reinstate with context: {ctx}")
...
def resolve_template_from_payload(payload: dict) -> Path:
    """Resolve the correct template for order reinstate."""
...
def generate_both_formats_from_payload(
    ...
    """
    Generate both DOCX and PDF for Order on Motion to Reinstate from payload data.
...
def main():
    sample = {
        ...
        "ChapterNumber": "13",
        "DocketNumber": "39",
        "TrusteeCalendar": "03/02/2026 at 10:00 AM",
        "X1": True,
        "X2": "",
        "X3": True,
    }
