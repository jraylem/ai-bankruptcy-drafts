...
DATAFILE = BASE_DIR / "data" / "payload.order_extend_regular.json"
OUT_DIR = BASE_DIR / "out"

TEMPLATE_REGULAR  = BASE_DIR.parent / "templates" / "order_granting_motion_to_extend.docx"
TEMPLATE_EXPEDITE = BASE_DIR.parent / "templates" / "order_granting_expedited_motion_to_extend.docx"

OUTPUT_BASENAME_REGULAR  = "Order_Granting_Motion_to_Extend_FILLED"
OUTPUT_BASENAME_EXPEDITE = "Order_Granting_Expedited_Motion_to_Extend_FILLED"
OUTPUT_BASENAME = OUTPUT_BASENAME_REGULAR  # default
...
def ensure_docx_template(template_path: Path | None = None) -> Path:
    p = template_path or TEMPLATE_REGULAR
    if p.exists():
        return p
...
def is_order_extend_expedite_from_order_extend_payload(payload: dict) -> bool:
    """Return True if payload indicates expedited."""
    val = payload.get("expedite") or payload.get("expedited")
    if val is None:
        return False
    if isinstance(val, bool):
        return val
    choice_raw = str(val).strip().lower()
    return choice_raw in ("expedite", "expedited", "true", "yes", "1")


def resolve_template_from_payload(payload: dict) -> Path:
    """Pick expedite or regular template based on payload."""
    if is_order_extend_expedite_from_order_extend_payload(payload):
        return ensure_docx_template(TEMPLATE_EXPEDITE)
    return ensure_docx_template(TEMPLATE_REGULAR)

def generate_document_from_payload(payload_data: dict, output_basename: str = None, output_type: str = "pdf") -> Path:
    is_expedite = is_order_extend_expedite_from_order_extend_payload(payload_data)
    default = OUTPUT_BASENAME_EXPEDITE if is_expedite else OUTPUT_BASENAME_REGULAR
    return generate_document(
        payload_data=payload_data,
        output_basename=output_basename,
        output_type=output_type,
        default_basename=output_basename or default,
...
def generate_pdf_from_payload_expedite(payload_data: dict, output_basename: str = None) -> Path:
    """Force the expedite template regardless of payload field."""
    template = ensure_docx_template(TEMPLATE_EXPEDITE)
    return generate_document(
        payload_data=payload_data,
        output_basename=output_basename,
        output_type="pdf",
        default_basename=output_basename or OUTPUT_BASENAME_EXPEDITE,
...
def generate_docx_from_payload_expedite(payload_data: dict, output_basename: str = None) -> Path:
    """Force the expedite template regardless of payload field."""
    template = ensure_docx_template(TEMPLATE_EXPEDITE)
    return generate_document(
        payload_data=payload_data,
...
def generate_both_formats_from_payload(payload_data: dict, output_basename: str = None) -> tuple[Path, Path]:
    """Generate granted version of the order. Returns (order_docx, order_pdf)."""
    is_expedite = is_order_extend_expedite_from_order_extend_payload(payload_data)
    template = resolve_template_from_payload(payload_data)
    ctx = build_context(payload_data, granted=True)
    default = OUTPUT_BASENAME_EXPEDITE if is_expedite else OUTPUT_BASENAME_REGULAR
    name_slug = output_basename or default
...
# -------------------- main --------------------
def main():
    template = ensure_docx_template(TEMPLATE_REGULAR)
...
if __name__ == "__main__":
    ...
    print("Testing order granting extend functionality...")
