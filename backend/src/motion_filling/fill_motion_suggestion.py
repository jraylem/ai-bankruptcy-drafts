"""
Motion to Suggestion filling functionality.
This module handles the creation and filling of motion to suggestion documents.
"""
...
TEMPLATE_CANDIDATES = [
    BASE_DIR.parent / "templates" / "suggestion_of_bankruptcy.docx",
]

OUTPUT_BASENAME = "Motion_to_Suggestion_FILLED"
...
def ensure_docx_template() -> Path:
    for p in TEMPLATE_CANDIDATES:
        if p.exists():
            return p
    raise SystemExit("Template not found. Place 'suggestion_of_bankruptcy.docx' under templates/ .")
...
def build_context(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ctx = {
        ...
        "CourtAgency": payload.get("CourtAgency", ""),
        "County": payload.get("County", "").upper(),
        "CircuitNumber": payload.get("CircuitNumber", "").upper(),
        "District": payload.get("District", ""),
        "BKCaseNumber": payload.get("BKCaseNumber", ""),
...
def resolve_template_from_payload(payload: dict) -> Path:
    """Resolve the template path. For motion suggestion, we use a single template."""
...
def generate_both_formats_from_payload(payload_data: dict, output_basename: str = None) -> tuple[Path, Path]:
    ...
    """
    print(f"INFO: Generating motion suggestion documents...")
...
if __name__ == "__main__":
    # Test the motion suggestion functionality
    test_payload = {
        "CaseNumber": "CACE22014610",
        "DebtorName": "Dorothy Robinson",
        "Creditor": "JPMorgan Chase Bank, National Association",
        "CourtAgency": "Miami-Dade County Court",
        "County": "Miami-Dade",
        "CircuitNumber": "11th",
        "District": "Southern",
        "BKCaseNumber": "25-21322",
        "DateFiled": "September 26, 2025",
    }
    
    print("Testing motion suggestion functionality...")
